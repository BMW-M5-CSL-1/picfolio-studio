...
class LocationDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('house', function ($location) {
                return [
                    $location->house
                ];
            })
            ->editColumn('shop', function ($location) {
                return [
                    $location->shop
                ];
            })
            ->editColumn('school', function ($location) {
                return [
                    $location->school
                ];
            })
            ->editColumn('park', function ($location) {
                return [
                    $location->park
                ];
            })
            ->editColumn('created_at', function ($location) {
                return Carbon::parse($location->created_at)->toDayDateTimeString();
            })
            ->editColumn('action', function ($location) {
                return '
                <div class="d-flex justify-content-cetner align-items-center" onclick="action_buttons(' . $location->id . ')">
                    <div class="btn-group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton100" id="">
                            ...
                            <div id="dropDownMenu_' . $location->id . '">
     ...
     */
    public function query(Location $location): QueryBuilder
    {
        return $location->newQuery()->orderBy('id', 'asc');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('house')->title('Houses')->addClass('text-nowrap')->orderable(true)->searchable(true),
            Column::computed('shop')->title('Shops')->addClass('text-nowrap')->orderable(true)->searchable(true),
            Column::computed('school')->title('Educational Institutes')->addClass('text-nowrap')->orderable(true)->searchable(true),
            Column::computed('park')->title('Parks')->addClass('text-nowrap')->orderable(true)->searchable(true),
