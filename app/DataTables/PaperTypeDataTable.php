...
class PaperTypeDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', 'papertype.action')
            ->addIndexColumn()
            ->editColumn('name', function ($paper_type) {
                return [
                    $paper_type->name
                ];
            })
            ->editColumn('guage', function ($paper_type) {
                return '<span class="text-lowercase">' . $paper_type->guage . ' mm</span>';
            })
            ->editColumn('type', function ($paper_type) {
                if ($paper_type->type == 'paper_media') {
                    return 'Paper Media';
                } elseif ($paper_type->type == 'vehicle_media') {
                    return 'Vehicle Media';
                } else {
                    return '-';
                }
            })
            ->editColumn('quality', function ($paper_type) {
                return [
                    $paper_type->paper_qualities->name
                ];
            })
            ->editColumn('rate', function ($paper_type) {
                return [
                    $paper_type->price
                ];
            })
            ->editColumn('side', function ($paper_type) {
                if ($paper_type->side == 'both') {
                    return 'Front & Back';
                } else {
                    return 'Only Front';
                }
            })
            ->editColumn('action', function ($paper_type) {
                return '
                <div class="d-flex justify-content-cetner align-items-center" onclick="action_buttons(' . $paper_type->id . ')">
                    <div class="btn-group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton100" id="">
                            ...
                            <div id="dropDownMenu_' . $paper_type->id . '">
            ...
            ->rawColumns(array_merge($columns, ['action', 'guage', 'type']))
            ->setRowId('id');
    }
     ...
     */
    public function query(PaperType $paper_type): QueryBuilder
    {
        return $paper_type->newQuery()->orderBy('id', 'asc');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('guage')->title('Paper Guage')->addClass('text-nowrap')->orderable(true)->searchable(true),
            Column::make('type')->title('Paper For')->addClass('text-nowrap')->orderable(true)->searchable(true),
            Column::make('quality')->name('paper_qualities.name')->title('Quality')->addClass('text-nowrap')->orderable(true)->searchable(true),
            Column::make('side')->title('Sides')->addClass('text-nowrap')->orderable(true)->searchable(true),
            Column::make('rate')->title('Rate')->addClass('text-nowrap')->orderable(true)->searchable(true),
            ...
            Column::make('action')->title('action')->addClass('text-nowrap')->orderable(false),
        ];
    }
